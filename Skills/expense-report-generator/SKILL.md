...
# 费用清单生成器 (Expense Report Generator)
...
2. **自动分类映射**：
   - 火车票 → 事由为 `出差交通(出发-目的)`，类别为 `长途交通费`。
   - 滴滴 → 事由为 `市内交通`，类别为 `市内交通费`。
3. **格式与美化**：
...
## 使用方法
直接运行 `scripts/generate_report.py` 脚本：
```bash
...
- 脚本会自动补全滴滴行程的年份（默认为当前年份）。
- 输出文件将使用 `assets/expense_template.xlsx` 中定义的样式。